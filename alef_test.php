<?php
  ...
  function randomInteger() {
    ...
    sumOfTheRestInts($assocArr, $assocArrRepeats, $indexedArr);
  }

  function sumOfTheRestInts($assocArr, $assocArrRepeats, $indexedArr) {    
    $assocArrSum = 0;
    for ($i = 0; $i < count($assocArr); $i++) {
      for ($j = 0; $j < count($assocArrRepeats); $j++) {
        if (!array_key_exists($assocArrRepeats[$indexedArr[$j]], $assocArr)) {
          $assocArrSum += $assocArr[$indexedArr[$i]];
        } 
      }
    }    
    echo "Sum: ".$assocArrSum;
  }
